...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(k=>Promise.all(k.filter(x=>x.startsWith('music-timeline-')&&x!==CACHE).map(x=>caches.delete(x)))).then(()=>self.clients.claim())));
